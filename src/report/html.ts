...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .save {
    ...
    border: 1px solid var(--line); color: var(--muted);
    /* "☆ save" and "★ saved" are different widths; without a floor the meta
       block on the right shifts as you click down the list. */
...
function card(r) {
  ...
  return '<div class="card' + (r.score >= 85 ? ' hot' : '') + (saved ? ' saved' : '') +
    ...
    '<span class="meta">' + esc(r.amountLabel) + (r.date ? ' · ' + esc(r.date) : '') +
